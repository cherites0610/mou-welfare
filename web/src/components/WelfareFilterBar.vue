...
const filterOptions = reactive({
    ...
    categories: ['兒童及青少年福利', '婦女與幼兒福利', '老人福利', '社會救助福利', '身心障礙福利', '其他福利'],
})
...
<template>
    <div class="sticky top-0 z-40 bg-gray-50">
            ...
            <div class="flex items-center justify-between px-4 py-3 text-sm text-gray-600">
                ...
                <div class="flex items-center gap-4 md:gap-6">
                    ...
                    <div class="relative">
                        <div @click="toggleDropdown('city')"
                            ...
                            :class="{ 'text-mygreen font-bold': activeDropdown === 'city' || selectedCities.length > 0 }">
                            <span>{{ cityButtonText }}</span>
                            <Icon icon="mingcute:down-line" class="transition-transform duration-300"
                        ...
                        <div v-if="activeDropdown === 'city'"
                            ...
                            <ul class="max-h-60 overflow-y-auto py-1">
                                <li v-for="city in filterOptions.cities" :key="city"
                                    ...
                                    <Icon
                                        :icon="selectedCities.includes(city) ? 'mingcute:checkbox-line' : 'mingcute:square-line'"
                                        class="text-xl"
                                        :class="selectedCities.includes(city) ? 'text-mygreen' : 'text-gray-300'" />
                                    <span :class="{ 'text-mygreen font-medium': selectedCities.includes(city) }">{{ city
                                        }}</span>
                                </li>
                            </ul>
                            <div class="p-2 border-t border-gray-100 ">
                                <button @click="clearSelection('city')"
                    ...
                    <div class="relative">
                        <div @click="toggleDropdown('category')"
                            class="flex items-center gap-1 cursor-pointer hover:text-gray-900 select-none"
                            :class="{ 'text-mygreen font-bold': activeDropdown === 'category' || selectedCategories.length > 0 }">
                        ...
                        <div v-if="activeDropdown === 'category'"
                            ...
                            <ul class="max-h-60 overflow-y-auto py-1">
                                <li v-for="cat in filterOptions.categories" :key="cat"
                                    @click="toggleSelection('category', cat)"
                                    class="px-4 py-2.5 flex items-center gap-3 cursor-pointer hover:bg-gray-50">
                                    <Icon
                                        :icon="selectedCategories.includes(cat) ? 'mingcute:checkbox-line' : 'mingcute:square-line'"
                                        class="text-xl"
                            ...
                            <div class="p-2 border-t border-gray-100 ">
                                <button @click="clearSelection('category')"
                                    class="w-full py-1.5 text-sm text-gray-500 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors font-medium flex items-center justify-center gap-1">
                                    <Icon icon="mingcute:delete-2-line" />
                                    清除篩選
                                </button>
                    ...
                    <div class="relative">
                        <div @click="toggleDropdown('family')"
                            class="flex items-center gap-1 cursor-pointer hover:text-gray-900 select-none"
                        ...
                        <div v-if="activeDropdown === 'family'"
                            ...
                            <ul class="max-h-60 overflow-y-auto py-1">
                                <li v-for="fam in familyStore.familyList" :key="fam.id" @click="selectFamily(fam.id)"
                            ...
                            <div class="p-2 border-t border-gray-100 ">
                                <button @click="selectFamily(null)"
                                    class="w-full py-1.5 text-sm text-gray-500 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors font-medium flex items-center justify-center gap-1">
                                    <Icon icon="mingcute:delete-2-line" />
                                    清除篩選
                                </button>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div @click="toggleAutoFilter"
                        ...
                        <Icon
                            :icon="isAutoFilter ? 'material-symbols:hdr-auto-rounded' : 'material-symbols:hdr-auto-outline-rounded'"
                            class="text-2xl" />
                    ...
                    <div @click="openFilter"
                        ...
                        <Icon :icon="selectedIdentities.length > 0 ? 'mingcute:filter-fill' : 'mingcute:filter-line'"
                            class="text-2xl" />
                        <span class="hidden md:flex">篩選</span>
                        <span v-if="selectedIdentities.length > 0"
                            class="flex md:hidden w-2 h-2 bg-red-500 rounded-full absolute top-3 right-3"></span>
                    </div>
                ...
                <div class="max-h-[60vh] overflow-y-auto p-5 space-y-3">
                    <div v-for="(section, idx) in advancedOptions" :key="idx">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <button v-for="opt in section.options" :key="opt" @click="toggleIdentity(opt)"
                                class="px-4 py-1.5 rounded-full text-sm font-medium border transition-all duration-200"
                                :class="tempIdentities.includes(opt) ? section.activeClass : `bg-white ${section.colorClass}`">
                                {{ opt }}
                            </button>
                        </div>
                    </div>
        ...
        <div v-if="isFilterOpen && !isDesktop" class="fixed inset-0 z-30 bg-black/20" @click="closeFilter"></div>
            ...
            <div v-if="isFilterOpen"
                ...
                <div class="bg-white rounded-2xl w-[500px] shadow-2xl flex flex-col max-h-[85vh]">
                    <div class="p-4 border-b border-gray-100 flex items-center justify-between">
                        ...
                        <button @click="closeFilter" class="p-1 hover:bg-gray-100 rounded-full transition-colors">
                    ...
                    <div class="p-4 border-t border-gray-100 flex gap-3">
                        <button @click="clearFilter"
                            class="flex-1 py-2.5 rounded-xl border border-mygreen text-mygreen font-bold hover:bg-green-50">
                            清除
                        </button>
                        <button @click="confirmFilter"
                            class="flex-1 py-2.5 rounded-xl bg-mygreen text-white font-bold shadow-lg shadow-green-200 hover:bg-green-600">
                            確定 ({{ tempIdentities.length }})
                        </button>
                    </div>
                </div>
